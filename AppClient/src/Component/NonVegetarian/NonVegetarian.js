...
const NonVegetarian = () => {
  ...
  useEffect(() => {
    axios.get("https://api.npoint.io/3c3c95781e7fc59922c1").then((res) => {
    ...
    <div>
     <nav className="navbar navbar-expand-lg NavBar">
      ...
      <div className='heading'>
        <h1>Ganesh Restaurant Non-Vegetarian Cuisines</h1>
      </div>
      <div className='vegetarian'>
        {data.map((item,idx)=>{
          return (
            ...
            <button onClick={(e)=>{CartHandler(item.name,item.item_image,item.price,item.description)}}>Add To Cart</button>
              <button onClick={(e)=>{orderHandler(item.name,item.item_image,item.description,item.price)}}>Order Now</button>
            </div>
          </div>
          )
        })}
