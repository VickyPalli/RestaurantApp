...
const Cart = () => {
  ...
  const orderHandler = (id,imageid,Desc,Price,Iden)=>{
    if(user){
      ...
      axios({
       ...
       data : input
        ...
        axios({
          url : `https://restaurant-server-vicky.herokuapp.com/user/delete/${Iden}`,
          method:"DELETE",
          headers:{
            authiazation : localStorage.getItem("Auth")
          },
   ...
   const deletehandle = (id)=>{
    axios({
      url : `https://restaurant-server-vicky.herokuapp.com/user/delete/${id}`,
      method:"DELETE",
      headers:{
        authiazation : localStorage.getItem("Auth")
      },
        ...
        {cart.map((item,idx)=>{
          return (
            ...
            <div className='cartbutton'>
              <button onClick={(e)=>deletehandle(item._id)}>Delete</button>
              <button onClick={(e)=>{orderHandler(item.Itemid,item.image,item.desc,item.price,item._id)}}>Order Now</button>
            </div>
          </div>
          )
        })}
